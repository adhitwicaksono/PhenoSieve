...
def match_module_entry(module_row: Dict[str, str], annotation_row: Dict[str, str]) -> Tuple[List[str], List[str]]:
    ...
    Matching is intentionally conservative in v0.1:
    - database-like identifiers are matched by exact case-insensitive token overlap;
    - symbols/aliases are matched by exact case-insensitive token overlap;
    - keywords are matched by substring search in annotation text fields.
    """
    ...
    # Keyword substring matching against annotation blob.
...
def match_modules(module_rows: List[Dict[str, str]], annotation_rows: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    One output row is emitted per unique:
    module + expected_gene + gene_id

    This prevents the same biological candidate from appearing multiple times simply
    because it matched through several evidence routes.
    """
    ...
    for module_row in module_rows:
        ...
        for ann in annotation_rows:
            ...
            key = (module, expected_gene, gene_id)

            status = "ambiguous" if matched_fields == ["keyword"] and len(matched_values) <= 1 else "candidate"
            ...
            if key not in merged:
                merged[key] = {
                    ...
                    "evidence_count": str(len(matched_values)),
                    "status": status,
                }
            else:
                old = merged[key]
                all_fields = sorted(set(split_multi(old["matched_fields"]) + matched_fields))
                all_values = sorted(set(split_multi(old["matched_values"]) + matched_values))
                old["matched_fields"] = ";".join(all_fields)
                old["matched_values"] = ";".join(all_values)
                old["evidence_count"] = str(len(all_values))
                old["status"] = "ambiguous" if all_fields == ["keyword"] and len(all_values) <= 1 else "candidate"
